...
def frame(label):
    ...
    d.text((44, H - 56), "Ledger & Loop Digital — The Small Business AI Prompt Playbook",
           font=load_font("arial.ttf", 24), fill=MUTED)
    return img, d


def build_inside_page():
    """A real category page from the guide: real title, real prompts, real tip."""
    img, d = frame("LOOK INSIDE — A REAL PAGE FROM THE GUIDE")
    cat = content.CATEGORIES[0]  # Marketing & Content
    px, py, pw, ph = 300, 110, 1000, 660
    d.rounded_rectangle([px, py, px + pw, py + ph], radius=18, fill=PAGE_BG)

    head_font = load_font("arialbd.ttf", 40)
...
def build_inside_tracker():
    """A mockup of the real companion spreadsheet's actual columns."""
    img, d = frame("LOOK INSIDE — THE COMPANION AI OPS TRACKER (.xlsx)")
    headers = ["Date", "Category", "Prompt Used", "Time Saved (min)", "Notes / Result"]
    sample_rows = [
        ["2026-08-03", "Marketing & Content", "Weekly Social Content Batch", "35", "5 posts from 1 topic, reused as-is"],
        ["2026-08-04", "Customer Service", "Difficult Email Response Drafter", "12", "Defused a refund complaint"],
        ["2026-08-05", "Sales & Proposals", "Discovery Call Follow-Up", "18", "Sent same day instead of next week"],
    ]
    gx, gy, gw = 200, 300, 1200
    col_w = [150, 220, 350, 220, 260]
    header_h = 76
    row_h = 64
    ...
    d.text((gx - 20, note_y),
           "Also includes: an ROI Summary tab that totals time saved by category and converts",
           font=note_font, fill=MUTED)
    d.text((gx - 20, note_y + 32),
           "it to dollars using your own hourly rate. Sample rows shown — yours starts blank.",
           font=note_font, fill=MUTED)
